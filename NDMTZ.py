...
#
# datos.m = len(lista)
# datos = Data([], m=datos.m, r=1, grid=True, tmax=600, alpha = True,
#              init=True,
#              show=True,
#              seed=2)
#
...
#
# grafos_T = Data([], m = 1, r = 6, grid = True, tmax=600, alpha = True,
#                 init = True,
#                 show = True,
#                 seed = 2)
# grafos_T.generar_grafo(4)
#
...
def NDMTZ(datos, grafo_data):
    ...
    # if datos.init:
    if datos.init:
        vals = NDMTZ_heuristic(datos, grafo_data)
        opcional = vals[3]
        iter = opcional[2]
        results = opcional[0:2]
        print(results)
        if isinstance(results[0], str):
            datos.tmax = 10
        else:
        # for g in T_index_prima:
            sols = MODEL.read('./soluciones/NDMTZ-HeurI{0}.sol'.format(iter))
        # except:
    ...
    MODEL.write('solucion_NDMTZ.sol')
...
#        plt.plot(xL[p, 0].X, xL[p, 1].X, 'ko', alpha = 0.5, markersize=5, color='yellow')
#        # ax.annotate("L" + str(i), xy = (xL[p, 0].X+0.5, xL[p, 1].X+0.5))
#        plt.plot(xR[p, 0].X, xR[p, 1].X, 'ko', markersize=2.5, color='blue')
#        # ax.annotate("R" + str(i), xy = (xR[p, 0].X-1.5, xR[p, 1].X-1.5))
#
#
#    ax.add_artist(Polygon(path_C, fill=False, closed = False, animated=False, linestyle='-', alpha=1, color='blue'))
#
#    for pathd in paths_D:
#        ax.add_artist(Polygon(pathd, fill=False, closed=False, lw = 1,
#                      animated=False, alpha=1, color='red'))
#
#    nx.draw(grafo.G, grafo.pos, node_size=100, node_color='black', alpha=1, edge_color='black')
#
#
...
#                node_color='black', alpha=0.3, edge_color='black')
#
#    plt.savefig('./images/NDMTZ-grid-mode{0}.png'.format(grafo_data.mode), dpi = 200)
