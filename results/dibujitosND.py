...
datos = pd.read_csv('./results/ND_results.csv')
datos
objval = datos[['Instance', 'List', 'GAP', 'Obj', 'Pct', 'Model', 'Mode']]
objval
# objval.groupby(['List']).plot(kind = 'bar', x = 'Structure', y = 'Time_h')
# datos2 = pd.read_table('results_MTZ_initval.txt')

# datos['Pct'] = (datos['ObjVal_h'] - datos['ObjVal'])/datos['ObjVal']
# datos2
#
...
#
g = sns.catplot(x = 'Model', y = 'GAP', hue = 'Mode', kind = 'box', col = 'Pct', row = 'List', data = objval, aspect = 0.5, sharey = True)
# g.savefig('time_h.png')

datos2 = pd.read_csv('./results/ND_results_init.csv')

# datos2 = pd.read_csv('./results/results_heuristic.csv')
objval_exact = datos2[['Instance', 'List', 'Structure', 'Pct', 'ObjVal', 'Time', 'GAP', 'ObjVal_h', 'Time_h', 'Model', 'Mode']]
improved_gap = (objval_exact['ObjVal_h'] - objval_exact['ObjVal'])/objval_exact['ObjVal']

objval_exact['gap_imp'] = improved_gap

g = sns.catplot(x = 'Structure', y = 'gap_imp', hue = 'Mode', kind = 'box', col = 'Pct', row = 'List', data = objval_exact, aspect = 0.5, sharey = True)
g.savefig('./images/improved_gap_ND.png')
#
# datos3 = pd.read_csv('./results/results_noinit.csv')
# datos3
# objval_exact_no = datos3[['Instance', 'List', 'Structure', 'Pct', 'ObjVal', 'Time', 'Gap']]
#
# gap_innoin = (objval_exact_no['ObjVal'] - objval_exact['ObjVal'])/objval_exact_no['ObjVal']
# objval_exact_no['gap_imp'] = gap_innoin
#
# g = sns.catplot(x = 'Structure', y = 'gap_imp', hue = 'Pct', kind = 'box', col = 'List', data = objval_exact_no, aspect = 0.5, sharey = True)
# g.savefig('differencewithwithout.png')
