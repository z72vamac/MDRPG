...
def NDMTZ_heuristic(datos, grafo_data):
    ...
    elipse = Data(elipses, m = len(elipses), grid = True, alpha = True, tmax = 1200, init = 0, show = True, seed = 2)
    ...
    iter = 0
    mode = 1
    xL, xR, obj = NDMTZ_aux(datos, grafo_data, fixed_data, iter)
    ...
    xL, xR, ugi, vgi, obj, path_2 = NDMTZ_aux(datos, grafo_data, fixed_data, iter)
    ...
    if datos.init:
        ...
        return z, xL_dict, xR_dict, ugi, vgi, results
    else:
...
def NDMTZ_aux(datos, grafo_data, fixed_data, iter):
    ...
    # grafos = Data([], m = 1, r = 6, grid = False, tmax=120, alpha = True,
    #                 init = True,
    #                 show = True,
    #                 seed = 2)
    # grafos.generar_grafo()
    #
    # grafo = grafos.mostrar_datos()[0]
    ...
    MODEL.addConstrs((pgLi.sum(g, '*') + pgij.sum(g, '*', '*') +  gp.quicksum(pgi[g, i]*grafos[g-1].longaristas[i // 100 - 1, i % 100] for i in grafos[g-1].aristas) + pgRi.sum(g, '*'))/datos.vD <= dLR[g]/datos.vC for g in T_index_prima)
    ...
    for e1, e2, g in deeLRg.keys():
        ...
        MODEL.addConstr(peeLRg[e1, e2, g] >= 0)
    ...
    for e1, e2, g1, g2 in deeRLgg.keys():
        ...
        MODEL.addConstr(peeRLgg[e1, e2, g1, g2] >= 0* zeeRLgg[e1, e2, g1, g2])
    ...
    MODEL.addConstrs(dRL[g1, g2] == gp.quicksum(peeRLgg[e1, e2, g1, g2] for e1 in grafo.aristas for e2 in grafo.aristas) for g1, g2 in z_index)
    ...
    # Origen y destino
    MODEL.addConstrs(xL[0, dim] == grafo_data.orig[dim] for dim in range(2))
    MODEL.addConstrs(xR[0, dim] == grafo_data.orig[dim] for dim in range(2))

    MODEL.addConstrs(xL[datos.m+1, dim] == grafo_data.dest[dim] for dim in range(2))
    MODEL.addConstrs(xR[datos.m+1, dim] == grafo_data.dest[dim] for dim in range(2))
    ...
    MODEL.write('./soluciones/NDMTZ-Heur.sol')
    ...
    plt.savefig('./imagenes/NDMTZ-heur-{0}.png'.format(iter))
