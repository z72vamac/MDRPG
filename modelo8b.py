...
MODEL.addConstrs((auxgLit[g, i, t, dim] >= xLt[t+1, dim] - Rgi[g, i, dim])
                 for g, i, t, dim in auxgLit.keys())
MODEL.addConstrs((auxgLit[g, i, t, dim] >= - xLt[t+1, dim]
                 + Rgi[g, i, dim]) for g, i, t, dim in auxgLit.keys())

MODEL.addConstrs(auxgLit[g, i, t, 0]*auxgLit[g, i, t, 0] + auxgLit[g, i, t, 1]
                 * auxgLit[g, i, t, 1] <= dgLit[g, i, t] * dgLit[g, i, t] for g, i, t in ugit.keys())
...
MODEL.addConstrs((auxgij[g, i, j, dim] >= Lgi[g, i, dim]
                 - Rgi[g, j, dim]) for g, i, j, dim in auxgij.keys())
MODEL.addConstrs((auxgij[g, i, j, dim] >= - Lgi[g, i, dim]
                 + Rgi[g, j, dim]) for g, i, j, dim in auxgij.keys())

MODEL.addConstrs(auxgij[g, i, j, 0]*auxgij[g, i, j, 0] + auxgij[g, i, j, 1]
                 * auxgij[g, i, j, 1] <= dgij[g, i, j] * dgij[g, i, j] for g, i, j in dgij.keys())
...
MODEL.addConstrs((auxgRit[g, i, t, dim] >= Lgi[g, i, dim] - xRt[t+1, dim])
                 for g, i, t, dim in auxgRit.keys())
MODEL.addConstrs((auxgRit[g, i, t, dim] >= - Lgi[g, i, dim]
                 + xRt[t+1, dim]) for g, i, t, dim in auxgRit.keys())

MODEL.addConstrs(auxgRit[g, i, t, 0]*auxgRit[g, i, t, 0] + auxgRit[g, i, t, 1]
                 * auxgRit[g, i, t, 1] <= dgRit[g, i, t] * dgRit[g, i, t] for g, i, t in vgit.keys())
...
longitudes = []
for g in range(nG):
    longitudes.append(sum([grafos[g].A[i // 100 - 1, i % 100]*grafos[g].longaristas[i // 100 - 1, i % 100] for i in grafos[g].aristas]))
...
MODEL.addConstrs((pgLit.sum('*', '*', t) + ugit.sum('*', '*', t)*pgij.sum(g, '*', '*') + ugit.sum('*','*', t)*longitudes[g]
                  + pgRit.sum('*', '*', t))/vD <= dLRt[t]/vC for t in T_index_prima)
...
for g, i in rhogi.keys():
    ...
    MODEL.addConstr(Rgi[g, i, 0] == rhogi[g, i] * grafos[g].V[first,
                    0] + (1 - rhogi[g, i]) * grafos[g].V[second, 0])
    MODEL.addConstr(Rgi[g, i, 1] == rhogi[g, i] * grafos[g].V[first,
                    1] + (1 - rhogi[g, i]) * grafos[g].V[second, 1])
    MODEL.addConstr(Lgi[g, i, 0] == landagi[g, i] * grafos[g].V[first,
                    0] + (1 - landagi[g, i]) * grafos[g].V[second, 0])
    MODEL.addConstr(Lgi[g, i, 1] == landagi[g, i] * grafos[g].V[first,
                    1] + (1 - landagi[g, i]) * grafos[g].V[second, 1])
...
objective = gp.quicksum(pgLit[g, i, t] + pgRit[g, i, t] for g, i, t in pgRit.keys()) + gp.quicksum(
    pgij[g, i, j] for g, i, j in pgij.keys()) + gp.quicksum(1*dLRt[t] + 1*dRLt[t] for t in T_index)
...
for path in paths_D:
    ax.add_artist(Polygon(path, fill=False, closed=False,
                  animated=False, alpha=1, color='black'))
