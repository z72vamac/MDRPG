...
def PDMTZ_heuristic(datos):
    ...
    elipse = Data(elipses, m = 6, grid = True, alpha = True, tmax = 1200, seed = 2)
    ...
    xL, xR, obj = PDMTZ_aux(datos, fixed_data, iter)
    ...
    xL, xR, ugi, vgi, obj, path_2 = PDMTZ_aux(datos, fixed_data, iter)

    # print(path_1)

    # xL, xR, obj = af.XPPNZ(datos, z, orig, datos.dest, 0)
    ...
    #     path_C = []
    #     for p in path_1:
    #         path_C.append(xL_dict[p])
    #         path_C.append(xR_dict[p])
    #
    #     plt.close()
    #     fig, ax = plt.subplots()
    #     plt.axis([0, 100, 0, 100])
    ...
    #     for g in T_index_prima:
    #         grafo = grafos[g-1]
    #         nx.draw(grafo.G, grafo.pos, node_size=5,
    #                 node_color='black', alpha=1, edge_color='black')
    #
    #     for p, i in zip(path_1, range(len(path_1))):
    #         ax.plot(xL_dict[p][0], xL_dict[p][1], 'o', color = 'blue', markersize = 3)
    #         if np.linalg.norm(np.array(xL_dict[p]) - np.array(xR_dict[p]) <= 0.5):
    #         # ax.annotate("$x_L^{0}$".format(i), xy = (xL_dict[p][0]-3, xL_dict[p][1]-3))
    #             ax.plot(xR_dict[p][0], xR_dict[p][1], 'o', color = 'black', markersize = 3)
    #         else:
    #             ax.plot(xL_dict[p][0], xL_dict[p][1], 'o', color = 'green', markersize = 3)
    #
    #
    #     ax.add_artist(Polygon(path_C, fill = False, linestyle = '-', alpha = 1, color = 'red'))
    #     # ax.add_artist(Polygon(path_P, fill = False, linestyle = '-', alpha = 1, color = 'red'))
    #
    #     plt.savefig('./imagenes/step2.png', dpi = 150)
    ...
    while not(all([i == j for i, j in zip(path_1, path_2)]) or all([i == j for i, j in zip(path_app, path_2)])):
        ...
        fixed_data = [xL_dict, xR_dict]
        xL, xR, obj, path_2 = PDMTZ_aux(datos, fixed_data, iter)
...
def PDMTZ_aux(datos, fixed_data, iter):
    ...
    MODEL.write('AMDRPG-MTZ-heuristic.lp')
